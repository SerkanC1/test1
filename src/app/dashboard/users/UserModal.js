...
export default function UserModal({ user, onClose, onSave }) {
  ...
  const validateForm = () => {
    ...
    if (!user) { // Yeni kullanıcı
      if (!formData.password) newErrors.password = 'Şifre gerekli'
      if (!formData.passwordConfirm) newErrors.passwordConfirm = 'Şifre tekrarı gerekli'
      if (formData.password !== formData.passwordConfirm) {
        newErrors.passwordConfirm = 'Şifreler eşleşmiyor'
      }
    } else if (formData.password || formData.passwordConfirm) { // Şifre değişikliği
      if (formData.password !== formData.passwordConfirm) {
        newErrors.passwordConfirm = 'Şifreler eşleşmiyor'
      }
    }
